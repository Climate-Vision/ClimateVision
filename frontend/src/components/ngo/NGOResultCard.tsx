          ...
          <div className="flex items-start justify-between gap-3">
            <div className="flex items-start gap-2">
              <svg className={cx(
                ...
                alert.severity === 'critical' || alert.severity === 'high' ? 'text-danger-500' :
                alert.severity === 'medium' ? 'text-amber-600' : 'text-base-400'
      ...
      {(onInvestigate || onExport) && (
        <div className="mt-4 pt-4 border-t border-base-800 flex items-center justify-end gap-2">
          ...
          {onExport && (
            <button
              onClick={onExport}
              className="px-3 py-1.5 text-sm font-medium text-white bg-brand-600 hover:bg-brand-500 rounded-lg transition"
            >
              Export Report
            </button>
          )}
